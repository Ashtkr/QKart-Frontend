...
const Products = () => {
  ...
  const performAPICall = async (text) => {
    let link = `${config.endpoint}/products`;
    if (text.length !== 0) {
      link = link + `/search?value=${text}`
    }
  ...
  const fetchCart = async (token) => {
    ...
    const link = `${config.endpoint}/cart`;
    try {
  ...
  const addToCart = async (
    ...
    if (!token) {
    ...
    } else if (isItemInCart(cartItem, productId) && options.preventDuplicate) {
      enqueueSnackbar(
      ...
      const link = `${config.endpoint}/cart`;
      const prod = { productId: productId, qty: qty };
      try {
        const res = await axios.post(link, prod, {
          headers: { Authorization: `Bearer ${token}` },
        });
